...
public class GlobalExceptionHandler {
    /**
     * Xử lý lỗi source map files (angular.min.js.map, bootstrap.min.css.map, etc.)
     * Không log lỗi để tránh spam logs khi bật DevTools
     */
    ...
    public ResponseEntity<Void> handleNoResourceFound(NoResourceFoundException ex) {
        ...
        if (resourcePath != null && (resourcePath.endsWith(".map") ||
                resourcePath.contains(".min.js.map") ||
                resourcePath.contains(".min.css.map"))) {
            return ResponseEntity.notFound().build();
        }

        // Log cho các static resource khác
     ...
     * Sửa để tránh HttpMediaTypeNotAcceptableException
     */
    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<String>> handleGeneralException(Exception ex) {
        // Kiểm tra nếu là lỗi HttpMediaTypeNotAcceptableException
        if (ex instanceof org.springframework.web.HttpMediaTypeNotAcceptableException) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE)
                    .body(ApiResponse.error("Định dạng phản hồi không được hỗ trợ"));
        }

...
import com.group7.ecommerce.service.AuthService;
...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<JwtResponse>> login(
        ...
        if (result.hasErrors()) {
            ...
            return ResponseEntity.badRequest().body(
                    ApiResponse.error("Dữ liệu không hợp lệ", result.getAllErrors())
            );
        }
        ...
        try {
            ...
            return ResponseEntity.ok(
                    ApiResponse.success("Đăng nhập thành công", jwtResponse)
            );
        } catch (IllegalArgumentException e) {
            // Email/Username hoặc mật khẩu không đúng
            return ResponseEntity.badRequest().body(
                    ApiResponse.error("Email/Username hoặc mật khẩu không đúng")
            );
        } catch (Exception e) {
            // Lỗi server khác
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.error("Có lỗi xảy ra: " + e.getMessage())
            );
        }
    }
    ...
    public ResponseEntity<String> products(){
        return ResponseEntity.ok("Hello product page");
    }

    @GetMapping("/order")
    public ResponseEntity<String> orders(){
        return ResponseEntity.ok("Hello order page");
    }
}